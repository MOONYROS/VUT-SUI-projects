...
void eval_strategy(std::unique_ptr<SearchStrategyItf>& search_strategy,
                   ...
                   StrategyEvaluation* report)
{
    ...
    if (in_progress.isFinal()) {
        ...
        report->time_taken +=
            std::chrono::duration_cast<decltype(report->time_taken)>(t1 - t0);
    }
    else {
        report->nb_failed++;
    }
...
std::unique_ptr<InitialStateProducerItf> getProducer(
...
std::unique_ptr<AStarHeuristicItf> getHeuristic(
    const argparse::ArgumentParser& parser)
{
...
std::unique_ptr<SearchStrategyItf> getSolver(
    const argparse::ArgumentParser& parser)
{
    ...
    if (solver_name == "dummy") {
        ...
        return std::make_unique<DepthFirstSearch>(
            parser.get<int>("--dls-limit"), parser.get<size_t>("--mem-limit"));
    }
...
int main(int argc, const char* argv[])
{
    ...
    parser.add_argument("--heuristic")
        .default_value(std::string("nb_not_home"));
    parser.add_argument("--dls-limit")
        .default_value(1'000'000)
        .scan<'d', int>();
    parser.add_argument("--mem-limit")
        .default_value(std::size_t { 2'147'483'648 })
        .scan<'u', size_t>();
    ...
    MemWatcher mem_watcher(parser.get<size_t>("--mem-limit"),
