...
std::vector<SearchAction> SearchState::actions() const {
    auto raw_moves =
        availableMoves(state_.non_homes.begin(), state_.non_homes.end(),
                       state_.all_storage.begin(), state_.all_storage.end());

    std::vector<SearchAction> moves;
    std::transform(
        raw_moves.begin(), raw_moves.end(), std::back_inserter(moves),
        [&](RawMove raw_move) {
            return SearchAction { locFromPtr(state_, raw_move.first),
                                  locFromPtr(state_, raw_move.second) };
        });
    return moves;
}
