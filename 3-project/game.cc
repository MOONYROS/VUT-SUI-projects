...
GameState::GameState(const GameState& other):
    homes(other.homes), free_cells(other.free_cells), stacks(other.stacks)
{
    recalculatePointerArrays_();
}
...
int moveCardsFromHomes(GameState* gs,
                       int max_nb_cards,
                       size_t stack_begin,
                       size_t stack_end,
                       std::default_random_engine rng)
{
    ...
    for (; nb_cards_moved < max_nb_cards; ++nb_cards_moved) {
        std::vector<CardStorage*> considered_froms {
            &gs->homes[nb_cards_moved % gs->homes.size()]
        };
        std::vector<CardStorage*> considered_tos = collect_location_pointers(
            gs->stacks.begin() + stack_begin, gs->stacks.begin() + stack_end);

        auto moves = availableMoves(considered_froms.begin(),
        ...
        int pick = std::uniform_int_distribution<std::mt19937::result_type>(
            0, moves.size() - 1)(rng);
        // we have a non-const access to GameState (gs), so we are allowed to
        // de-const the pointers in RawMove
        move(const_cast<CardStorage*>(moves[pick].first),
             const_cast<CardStorage*>(moves[pick].second));
    }
...
Location locFromPtr(const GameState& gs, const CardStorage* ptr)
{
    if (isInContainer(ptr, gs.homes))
        ...
        return { LocationClass::FreeCells,
                 positionInContainer(ptr, gs.free_cells) };
    else
        throw std::out_of_range("Pointer doesn't match any of homes, freecells "
                                "or stacks in the given GameState");
}
...
std::optional<std::pair<CardStorage*, WorkStack*>> findIrreversibleMove(
    GameState* gs,
    std::default_random_engine& rng)
{
    ...
    int pick_from = std::uniform_int_distribution<std::mt19937::result_type>(
        0, possible_from.size() - 1)(rng);
    ...
    int pick_to = std::uniform_int_distribution<std::mt19937::result_type>(
        0, possible_to.size() - 1)(rng);
    auto to = possible_to[pick_to];

    return std::make_pair(from, to);
}

auto findHomeFor(const GameState& gs,
                 Card card) -> decltype(gs.homes)::const_iterator
{
    return std::find_if(
        gs.homes.begin(), gs.homes.end(), [&](const CardStorage& cs) {
            return cs.canAccept(card);
        });
}
...
bool cardIsHome(const GameState& gs, Card card)
{
    for (const auto& home : gs.homes) {
        ...
        if (opt_top_card->color == card.color
            && opt_top_card->value >= card.value)
            return true;
    }
...
{
    os << "Homes: " << state.homes[0] << " " << state.homes[1] << " "
       << state.homes[2] << " " << state.homes[3] << "\n";

    os << "FreeCells: " << state.free_cells[0] << " " << state.free_cells[1]
       << " " << state.free_cells[2] << " " << state.free_cells[3] << "\n";
